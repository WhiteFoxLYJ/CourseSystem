...
<html>
  <head>
	...
	<script type="text/javascript" src="js/StudentCourseSelect.js"></script>
	<script type="text/javascript">
	function getName(bu) {
		var i=bu.id;
		alert("you click button:"+i); 
		$.getJSON("StuSelectCourseT?coursetId="+i,function(Selmessage){
			if(Selmessage=="0"){
				alert("选课成功，请到选课信息查询界面查询。");
  ...
  <body>
    学生选课界面 StudentCourseSelect.jsp<hr>
   欢迎	<%=ActionContext.getContext().getSession().get("login_name") %>登录<br>
   您的学号为<%=ActionContext.getContext().getSession().get("login_id") %>
    <hr>
      <table id="SelectCourseTable" style =" display: ;margin:0 auto;height: auto; width: 800px" border="1">
      	<tr>
      		<td>课程编号</td>
      		<td>课程名称</td>
      		<td>修读类型</td>
